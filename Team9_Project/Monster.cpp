...
Monster::Monster(const string& name, int level, int gold, int exp)
	: name(name), dropGold(gold), dropEXP(exp)
{
	...
	void Monster::GetHit(int damage)
	{
		HP -= damage;
		if (HP < 0) HP = 0;
		cout << name << "이(가) " << damage << " 피해를 입었습니다. 남은 HP: " << HP << endl;
	}
	...
	int Monster::getAttack() const { return ATK; }
	int Monster::getDropGold() const { return dropGold; }
