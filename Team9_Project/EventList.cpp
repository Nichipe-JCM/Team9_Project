...
void FixCodeEvent::EventEffect(Character* character, GameManager* gm) {
	...
	cout << Color::BRIGHT_WHITE << "휴 드디어 끝냈다. 제출하기 전에 오타는 없는지 다시 확인해봐야지" << Color::RESET << endl;
	cout << Color::BRIGHT_WHITE << "어우 내가 짠 코드가 이렇게 더러웠었나.. 심각하네." << Color::RESET << endl;
	cout << Color::BRIGHT_WHITE << "다시보니까 다시 수정하면 훨씬 간단하게 바꿀 수 있겠는데?" << Color::RESET << endl;
	cout << Color::BRIGHT_WHITE << "하루 정도 시간도 남았는데 수정해볼까?" << Color::RESET << endl;
	...
	while (true) {
		...
		if (select == 1) {
			if (RandNum == 0 || RandNum == 1) {
				...
				cout << Color::BRIGHT_WHITE << "역시 함수와 클래스를 잘 활용하니까 코드가 훨씬 짧아지네" << Color::RESET << endl;
			...
			cout << Color::BRIGHT_WHITE << "일단 이 상태로 제출하고 나중에 궁금한 점이 생기면 인터넷을 찾아보자." << Color::RESET << endl;
...
void SOSRequestEvent::EventEffect(Character* character, GameManager* gm) {
	...
	cout << Color::BRIGHT_WHITE << "팀원: 팀장님 지금 작성한 코드에서 오류가 너무 많이나와요.. 도와주세요 ㅜㅜ " << Color::RESET << endl;
	...
	while (true) {
		...
		if (select == 1) {
			int loss = static_cast<int>(character->getMaxHP() * 0.025);
			int gain = character->getLevel() * 4;
...
void YouTubeAlgorithmEvent::EventEffect(Character* character, GameManager* gm) {
	...
	while (true) {
		...
		if (select == 1) {
			...
			int gain = character->getLevel() * 2;
			cout << Color::BRIGHT_WHITE << "지금은 참아야 한다." << Color::RESET << endl;
...
void RelayQuizEvent::EventEffect(Character* character, GameManager* gm) {
	...
	cout << Color::CYAN << "\"띠링!\"" << Color::BG_BRIGHT_WHITE <<  " Slack에 알람이 울린다." << Color::RESET << endl;
